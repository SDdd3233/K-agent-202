...
These tools are not bundled with this package. Use them only when the current
environment independently provides a `paper-search` MCP server; do not describe
them as guaranteed capabilities of the bundled `academic-search` server.
