"""Search-only ScienceDirect data source via pybliometrics."""
...
class ScienceDirectSource:
    """Search ScienceDirect article metadata and normalize the first page."""
...
def _links(value: Any) -> dict[str, str]:
    ...
    for item in _as_list(value):
        if not isinstance(item, dict):
            continue
        reference = item.get("@ref") or item.get("rel")
        href = item.get("@href") or item.get("href")
        if reference and href:
            links[reference] = href
    return links
