...
当前版本为 v0.3.2。仓库可以作为带 MCP 安装后验证的可移植技能包使用；如果 MCP 初始化或注册后的可见性验证失败，安装器会直接失败，不会把不完整安装报告为成功。文档化的求解器基线是 LS-DYNA R14.1.1，内置手册参考为 R16。材料值属于文献或手册中的代表性值，正式工程结论应使用项目实测数据替换或标定。
